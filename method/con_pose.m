function [poseRXpro,posetXpro,poseRYpro,posetYpro]=con_pose(eRibn,etibn,qijn,pattern,K)
...
while (step_th<step && k_num<iter_max)
    ...
    for i=1:n
        alpha=RX_k*(RA(:,:,i)*tY_k+tA(:,i));
        J=[-skew(alpha) eye(3) RX_k*RA(:,:,i)];
        f=alpha+tX_k;
...
posetXpro=tX_k_list(:,end)*1000;
poseRYpro=rod2dcmv(RY_k_list(:,end));
posetYpro=tY_k_list(:,end)*1000;
